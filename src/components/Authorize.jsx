...
const Authorize = (props) => {
  ...
  useEffect(() => {
    const checkForToken = async () => {
      ...
      if (
        localStorage.trelloToken &&
        (await isValidToken(localStorage.trelloToken))
      ) {
        context.keys.token = localStorage.trelloToken;
        props.setIsAuthorized(!props.isAuthorized);
      } else if (matchIndex !== -1) {
        if (window.location.href.indexOf("error=") === -1) {
          localStorage.trelloToken = window.location.href.substring(
            matchIndex + "#token=".length
          );
          context.keys.token = localStorage.trelloToken;
          window.window.location.href = window.location.href.substring(
            0,
            matchIndex
          );
          props.setIsAuthorized(!props.isAuthorized);
      ...
      {isLoaded && (
        <div className="authorize">
          ...
          <div className="authorize__text">
            <h3>
              In order to use it, we will require you to first authorize the
              app.
            </h3>
            <p>
              Please click the button below to be redirected to the Trello
              authorization page.
            </p>
            <p>
              This Application will request the following access:{" "}
              <span className="authorize__text-bold">
                Read & Write to your boards for 1 hour.
              </span>
            </p>
            <p>
              Once you authorize the Application, you will be redirected back
              here.
            </p>
          </div>
          <button
            className="authorize__button"
            onClick={() => authAPI.authorizeApp(context.keys.apiKey)}
          >
            Authorize
          </button>
